...
def KeyExpansion(key, expandedKey):
    ...
    while len(expandedKey) < 176:
        temp_word = expandedKey[-4:]

        if len(expandedKey) % currentSize == 0:
            temp_word = KeyExpansionCore(temp_word, rconIter)
            rconIter += 1

        for i in range(4):
            expandedKey.append(expandedKey[-currentSize] ^ temp_word[i])
...
def ShiftRows(state):
    ...
    for r in range(4):
        for c in range(4):
            new_state[r + 4 * c] = state[(r + c) % 4 + 4 * c]
...
def AES(text, key):
    ...
    for round in range(numOfRounds):
        ...
        ShiftRows(state)
        MixColumns(state)
        AddRoundKey(state, expandedKey[16 * (round + 1): 16 * (round + 2)])

    SubBytes(state)
    ShiftRows(state)
    AddRoundKey(state, expandedKey[160: 176])

    encrypted_text = "".join(format(b, '02x') for b in state)
...
orgLen = len(text)
lenOfPaddedText = orgLen

if orgLen % 16 != 0:
    lenOfPaddedText = ((lenOfPaddedText // 16) + 1) * 16

paddedText = [0] * lenOfPaddedText
for i in range(orgLen):
    paddedText[i] = ord(text[i])

for i in range(0, lenOfPaddedText, 16):
    encrypted_block = AES(paddedText[i:i + 16], key)
    paddedText[i:i + 16] = [int(encrypted_block[j:j+2], 16) for j in range(0, len(encrypted_block), 2)]

encrypted_text = "".join(format(b, '02x') for b in paddedText)
